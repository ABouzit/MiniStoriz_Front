...
class AllUsers extends React.Component {
  ...
  componentDidMount() {
    ...
    if (!user) {
      ...
      if (typeof this.props.match.params.userId === 'undefined') {
        this.setState({  connected: true,idUser: user.id }, ()=> {this.fetchUser();this.fetchUsers();this.forceUpdate()});
         } else {
        this.setState({  connected: true,idUser: this.props.match.params.userId,idCurrentUser: user.id }, ()=> {this.fetchUser();this.fetchRelation(this.props.match.params.userId);this.fetchUsers();subscriber.next({view:this.props.match.params.userId});this.forceUpdate()});
      }
    }
  ...
  fetchRelation(id) {
    Axios.get(config.API_URL + "relations/getRelationId/" + this.state.idCurrentUser+"/"+id, {}).then(res => {
  ...
  fetchUsers() {
    ...
    Axios.get(
      config.API_URL +
        "users/relations/xxxx/"+this.state.user.id,
      {}
    ).then(res => {
      console.log(res.data.length)
      if (res.data.length > 6) {
        this.setState({ users: res.data,numberPage: res.data.length,showMore: true });
      } else {
  ...
  deleteRequest(id){
    ...
    Axios.delete(
      config.API_URL +
        "relations/between/"+currentId+"/"+id 
    ).then(res => {
  ...
  searchCheck() {
    this.setState({ page: 1 }, () => {
        if (this.state.search !== "") {
          Axios.get(
            config.API_URL +
              "users/relations/"+this.state.search+"/"+this.state.user.id,
            {}
          ).then(res => {
            this.setState({ users: res.data,numberPage: res.data.length });
          });
            ...
            <div>
              <GridContainer justify="center" spacing={"auto"}>
                {this.state.users.map((user, index) => {
                  if(index < this.state.page * 6 && user.id !== this.state.idCurrentUser)
                  return (
                    ...
                    >
                        ...
                        >
                          <Parallax
                            image={
                              user.lienCouverture !== null ? user.lienCouverture : config.API_URL + "images/asset/bg1.jpg"
                            }
                            style={{
                              height: "170px",
                              marginLeft: "auto",
                        ...
                        <Divider
                          ...
                          <GridContainer justify="center" style={{ marginTop: 8 }}>
                            ...
                            <GridItem xs={4} sm={4} md={4} style={{ textAlign: "center" }}>
                                ...
                                <ButtonBase onClick={()=>{this.deleteRequest(user.id)}}>
                                  <small style={{color: '#1e1548'}}>
    ...
    <Card style={{ backgroundColor: "white" }}>
      ...
      >
        ...
        <GridContainer justify="center" style={{ marginTop: "7%" }}>
          ...
          <GridItem xs={4} sm={4} md={4}>
            <ButtonBase onClick={()=>{this.requestFriend(user.id)}}>
              <small style={{color: '#1e1548'}}>
